...
CREATE PROCEDURE ESC_AGREGAR_USUARIO (
...
CREATE PROCEDURE ESC_ACTUALIZAR_ROL_USUARIO (
    IN p_id_usuario INT,
...
CREATE PROCEDURE ESC_ACTUALIZAR_USUARIO (
    IN p_id_usuario INT,
    IN p_nombre VARCHAR(1000),
    IN p_email VARCHAR(100),
    IN p_pais VARCHAR(50)
...
CREATE PROCEDURE ESC_ACTUALIZAR_ESTADO_USUARIO (
    IN p_id_usuario INT,
...
CREATE PROCEDURE LEC_CONSULTAR_USUARIO_ACTIVO ()
BEGIN
...
CREATE PROCEDURE ESC_ASIGNAR_PLAN_USUARIO (
    IN p_id_usuario INT,
    IN p_id_plan INT
)
BEGIN
...
CREATE PROCEDURE ESC_REGISTRAR_PAGO (
    IN p_metodo VARCHAR(50),
    IN p_monto DECIMAL(10,2),
    IN p_id_usuario INT
)
BEGIN
    INSERT INTO HISTORIAL_PAGO (metodo, monto, fecha_pago, estado, id_usuario)
    VALUES (p_metodo, p_monto, CURDATE(), 'PENDIENTE', p_id_usuario);
END //

CREATE PROCEDURE ESC_CONFIRMAR_PAGO (
    IN p_id_pago INT
)
BEGIN
...
CREATE PROCEDURE LEC_OBTENER_PAGO_PENDIENTE ()
BEGIN
    SELECT 
        H.id_pago,
        U.nombre AS usuario,
...
CREATE PROCEDURE LEC_TOTAL_PAGO_USUARIO (
    IN p_id_usuario INT
)
BEGIN
    SELECT
...
CREATE PROCEDURE LEC_RESUMEN_PAGO_POR_USUARIO ()
BEGIN
    SELECT 
        U.id_usuario,
        U.nombre,
...
-- ---------------------------------------------------------
-- PROCEDIMIENTOS DE SERVIDORES Y JUEGOS
-- ---------------------------------------------------------

CREATE PROCEDURE ESC_CREAR_SERVIDOR (
    IN p_nombre VARCHAR(1000),
    IN p_dominio VARCHAR(100),
    IN p_puerto INT,
    IN p_estado VARCHAR(50),
    IN p_id_variacion INT
)
BEGIN
    INSERT INTO SERVIDOR (nombre, dominio, puerto, estado, id_variacion)
    VALUES (p_nombre, p_dominio, p_puerto, p_estado, p_id_variacion);
END //

CREATE PROCEDURE ESC_REINICIAR_SERVIDOR (
...
CREATE PROCEDURE ESC_ACTUALIZAR_SERVIDOR (
    IN p_id_servidor INT,
    IN p_nombre VARCHAR(1000),
    IN p_dominio VARCHAR(100),
    IN p_puerto INT
...
CREATE PROCEDURE ESC_ELIMINAR_SERVIDOR (
    IN p_id_servidor INT
)
BEGIN
...
CREATE PROCEDURE LEC_SERVIDOR_POR_JUEGO ()
BEGIN
    SELECT 
        J.nombre AS juego,
...
CREATE PROCEDURE LEC_SERVIDOR_POR_VARIACION ()
BEGIN
    SELECT 
        J.nombre AS juego,
        V.nombre_variacion AS variacion,
        COUNT(S.id_servidor) AS cantidad_servidores
    FROM VARIACION_JUEGO V
    JOIN JUEGO J ON V.id_juego = J.id_juego
    LEFT JOIN SERVIDOR S ON V.id_variacion = S.id_variacion
    GROUP BY V.id_variacion, J.nombre, V.nombre_variacion;
END //

-- ---------------------------------------------------------
-- PROCEDIMIENTOS DE TICKETS Y SOPORTE
-- ---------------------------------------------------------

CREATE PROCEDURE ESC_CREAR_TICKET (
...
CREATE PROCEDURE ESC_CERRAR_TICKET (
    IN p_id_ticket INT
)
BEGIN
    UPDATE TICKET
...
CREATE PROCEDURE LEC_TICKET_POR_USUARIO (
    IN p_id_usuario INT
)
BEGIN
    SELECT *
